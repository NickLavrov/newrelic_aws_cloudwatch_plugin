module NewRelicAWS
  module Collectors
    class EC2 < Base
      ...
      #     tagged_instances
      #   else
      #     @ec2.instances({filters: [{name: 'tag:Accounting', values: ['Production']}]})
      #   end
      # end
      ...
      def collect
        data_points = []
        @ec2.instances({filters: [{name: 'tag:Accounting', values: ['Production']}]}).each do |instance|
          puts 'ID:    ' + instance.id
          ...
          metric_list.each do |(metric_name, statistic, unit)|
            ...
            data_point = get_data_point(
              ...
              :component_name => name_tag.nil? ? instance.id : "#{name_tag} (#{instance.id})"
            )
